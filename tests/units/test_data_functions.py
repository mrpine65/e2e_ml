...
def test_create_inmm_feature():
    """
    Unit case to test the function that creates the EIdeal Number of Main Meals? (INMM) feature.
    """
    _dataset = dataset.copy()

    assert "INMM" not in _dataset.columns.tolist()

    _dataset = _create_inmm_features(dataframe=_dataset)

    assert "INMM" in _dataset.columns.tolist()
    assert isinstance(_dataset["INMM"].dtype, type(np.dtype("object")))
...
def test_transform_numerical_columns():
    ...
    numerical_columns = _dataset2.select_dtypes(include="number").columns.tolist()
